...
/>
  ...
  },
  {
    ...
    desc:`Что это:
Кастомный курсор, который следует за мышью с плавной задержкой.
Состоит из двух элементов: точки (следует мгновенно) и кольца (следует с интерполяцией).
При наведении на интерактивные элементы курсор увеличивается.

Как работает:

Использует requestAnimationFrame для плавной анимации.

Точка следует за курсором мгновенно через transform: translate().

Кольцо следует за точкой с lerp-интерполяцией (коэффициент 0.12).

Отслеживает hover над интерактивными элементами через closest().

При наведении: точка увеличивается в 1.5x, кольцо — в 1.8x.

Скрывает стандартный курсор через cursor: none на body.

Что внутри:

Два fixed-элемента: .cursor_dot и .cursor_ring.

useEffect с animation loop (requestAnimationFrame).

Слушатели mousemove, mouseover, mouseout, mousedown, mouseup.

Refs для прямого управления DOM без re-render.`,
    code: `import CursorFollower from './CursorFollower';
